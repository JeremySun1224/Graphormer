...
class GraphormerModel(FairseqEncoderModel):
    def __init__(self, args, encoder):
        ...
        self.encoder_embed_dim = args.encoder_embed_dim  # zinc 80
    ...
    def build_model(cls, args, task):
        ...
        if not safe_hasattr(args, "max_nodes"):  # 检查args对象是否具有max_nodes属性
        ...
        return cls(args, encoder)  # 创建模型实例

    def forward(self, batched_data, **kwargs):
        # logger.info(f"batched_data: {batched_data.keys()}")
        # logger.info(f"batched_data: {batched_data}")
        # batched_data: ['idx', 'attn_bias', 'attn_edge_type', 'spatial_pos', 'in_degree', 'out_degree', 'x', 'edge_input', 'y']
        # exit()
