...
class GraphPredictionTask(FairseqTask):
    ...
    def __init__(self, cfg):
        ...
        if cfg.user_data_dir != "":
            ...
            print(f"第1步, graph prediction task 走到了这里!!!!!!!!!!")
            self.dm = GraphormerDataset(
...
class GraphPredictionWithFlagTask(GraphPredictionTask):
    ...
    def train_step(
        ...
        batched_data = sample["net_input"]["batched_data"]["x"]  # [n_graph, n_node, 1]
        n_graph, n_node = batched_data.shape[:2]
        perturb_shape = n_graph, n_node, model.encoder_embed_dim  # [n_graph, n_node, n_hidden]
        if self.flag_mag > 0:
            perturb = (
                torch.FloatTensor(*perturb_shape)
                .uniform_(-1, 1)
                .to(batched_data.device)
            )  # 与batched_data形状相同, 初始化为[-1, 1]的均匀分布的扰动
            perturb = perturb * self.flag_mag / math.sqrt(perturb_shape[-1])  # TODO
        else:
            perturb = (
                torch.FloatTensor(*perturb_shape)
                .uniform_(-self.flag_step_size, self.flag_step_size)
                .to(batched_data.device)
            )  # 与batched_data形状相同, 初始化为[-flag_step_size, flag_step_size]的均匀分布的扰动
