...
class PygPCQM4Mv2Dataset(InMemoryDataset):
    ...
    @property
    def raw_file_names(self):  # must be present in order to skip downloading
        return 'data.csv.gz'

    @property
    def processed_file_names(self):  # must be present in order to skip processing
        return 'geometric_data_processed.pt'

    def download(self):
        if decide_download(self.url):
            path = download_url(self.url, self.original_root)  # Downloading https://dgl-data.s3-accelerate.amazonaws.com/dataset/OGB-LSC/pcqm4m-v2.zip
            extract_zip(path, self.original_root)  # Extracting /root/sx/sx_pytorch_new/Graphormer/datasets/pcqm4m-v2.zip
            os.unlink(path)
        else:
            print('Stop download.')
            exit(-1)
    ...
    def process(self):
        ...
        # data_df = data_df.iloc[:1000, :]  # TODO: 取前1000条测试
        ...
        for i in tqdm(range(len(smiles_list))):
            data = Data()  # torch.geometric.Data
