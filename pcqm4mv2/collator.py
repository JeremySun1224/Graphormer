...
def collator(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20):
    ...
    y = torch.cat(ys)  # torch.cat(dim=0), 默认按行拼接
    x = torch.cat([pad_2d_unsqueeze(i, max_node_num) for i in xs])  # n_node pad
...
if __name__ == '__main__':
    _x = torch.tensor([1, 2, 3, 4], dtype=torch.long)
    _padlen = 8
    _pad_x = pad_1d_unsqueeze(x=_x, padlen=_padlen)
    print(_pad_x)
